...
--[[
    Remote Spy Module
    Tüm RemoteEvent, RemoteFunction, BindableEvent, BindableFunction çağrılarını izler.
]]
...
local function main()
    local RemoteSpy = {}
    local window, statusLabel, toggleButton
    local isActive = false
    local connections = {}  -- bağlantıları tutar
    local outputLines = {}  -- konsol çıktısı için
    ...
    local function createGui()
        ...
        -- Ana frame
        local mainFrame = Instance.new("Frame", window.GuiElems.Content)
        mainFrame.Size = UDim2.new(1, 0, 1, 0)
        mainFrame.BackgroundTransparency = 1

        -- Durum etiketi
        statusLabel = Lib.Label.new()
        statusLabel.Gui.Parent = mainFrame
        statusLabel.Gui.Position = UDim2.new(0, 10, 0, 10)
        statusLabel.Gui.Size = UDim2.new(0, 200, 0, 20)
        statusLabel.Gui.Text = "Durum: Kapalı"
        statusLabel.Gui.TextColor3 = Color3.fromRGB(200, 200, 200)

        -- Aç/Kapa butonu
        toggleButton = Lib.Button.new()
        toggleButton.Gui.Parent = mainFrame
        toggleButton.Gui.Position = UDim2.new(0, 10, 0, 40)
        toggleButton.Gui.Size = UDim2.new(0, 100, 0, 25)
        toggleButton.Gui.Text = "Başlat"
        toggleButton.OnClick:Connect(function()
            if isActive then
                RemoteSpy.Stop()
            else
                RemoteSpy.Start()
        ...
        -- Çıktı metin kutusu (ScrollingFrame içinde)
        ...
        local padding = Instance.new("UIPadding", outputFrame)
        padding.PaddingLeft = UDim.new(0, 5)
        padding.PaddingRight = UDim.new(0, 5)
        padding.PaddingTop = UDim.new(0, 5)

        -- Örnek çıktı satırı (şablon)
        ...
        RemoteSpy.Gui = window
        RemoteSpy.OutputFrame = outputFrame
        RemoteSpy.Template = template
    end

    -- Mesaj ekleme fonksiyonu
    local function addOutput(text, color)
        color = color or Color3.fromRGB(200, 200, 200)
        local newLabel = RemoteSpy.Template:Clone()
        newLabel.Text = text
        newLabel.TextColor3 = color
        newLabel.Visible = true
        newLabel.Parent = RemoteSpy.OutputFrame
        RemoteSpy.OutputFrame.CanvasSize = UDim2.new(0, 0, 0, RemoteSpy.OutputFrame.UIListLayout.AbsoluteContentSize.Y + 10)
        -- Otomatik aşağı kaydır
        RemoteSpy.OutputFrame.CanvasPosition = Vector2.new(0, 9e9)
    end
    ...
    function RemoteSpy.Start()
        ...
        isActive = true
        toggleButton.Gui.Text = "Durdur"
        statusLabel.Gui.Text = "Durum: Aktif"
        statusLabel.Gui.TextColor3 = Color3.fromRGB(0, 255, 0)
        addOutput("Remote Spy başlatıldı.", Color3.fromRGB(0, 255, 0))
        ...
        local function hookRemote(remote)
            ...
            if remoteType == "RemoteEvent" then
                ...
                if oldEvent then
                    local con = remote.OnServerEvent:Connect(function(player, ...)
                        local args = {...}
                        local argStr = ""
                        for i, v in ipairs(args) do
                            argStr = argStr .. tostring(v) .. (i < #args and ", " or "")
                        end
                        addOutput(string.format("[RemoteEvent] %s -> %s (%s)", remote.Name, player.Name, argStr), Color3.fromRGB(255, 200, 100))
                    end)
                    connections[remote] = con
                end
                -- FireServer (invoke) yakalanamaz, çünkü fonksiyon çağrısıdır, event değil. Ancak hookfunction ile yapılabilir.
            elseif remoteType == "RemoteFunction" then
                -- OnServerInvoke
                local oldInvoke = remote.OnServerInvoke
                if oldInvoke then
                    -- OnServerInvoke bir fonksiyondur, onu hooklayabiliriz.
                    local function newInvoke(player, ...)
                        local args = {...}
                        local argStr = ""
                        for i, v in ipairs(args) do
                            argStr = argStr .. tostring(v) .. (i < #args and ", " or "")
                        end
                        addOutput(string.format("[RemoteFunction] %s <- %s (%s)", remote.Name, player.Name, argStr), Color3.fromRGB(100, 200, 255))
                        return oldInvoke(player, ...)
                    end
                    remote.OnServerInvoke = newInvoke
                    connections[remote] = {type = "function", old = oldInvoke} -- tut
                end
            elseif remoteType == "BindableEvent" then
                -- Fire
                local oldFire = remote.Fire
                if oldFire then
                    local con = remote.Event:Connect(function(...)
                        local args = {...}
                        local argStr = ""
                        for i, v in ipairs(args) do
                            argStr = argStr .. tostring(v) .. (i < #args and ", " or "")
                        end
                        addOutput(string.format("[BindableEvent] %s fired (%s)", remote.Name, argStr), Color3.fromRGB(200, 255, 200))
                    end)
                    connections[remote] = con
                end
            elseif remoteType == "BindableFunction" then
                ...
                if oldInvoke then
                    local function newInvoke(...)
                        local args = {...}
                        local argStr = ""
                        for i, v in ipairs(args) do
                            argStr = argStr .. tostring(v) .. (i < #args and ", " or "")
                        end
                        addOutput(string.format("[BindableFunction] %s invoked (%s)", remote.Name, argStr), Color3.fromRGB(255, 150, 255))
                        return oldInvoke(...)
                    end
                    remote.Invoke = newInvoke
                    connections[remote] = {type = "function", old = oldInvoke}
                end
            end
        end

        -- Mevcut remoteları tara
        ...
        -- Yeni eklenenleri izle
        connections._descendantAdded = game.DescendantAdded:Connect(function(obj)
            if obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction") or obj:IsA("BindableEvent") or obj:IsA("BindableFunction") then
                hookRemote(obj)
            end
        end)
    ...
    function RemoteSpy.Stop()
        ...
        statusLabel.Gui.Text = "Durum: Kapalı"
        statusLabel.Gui.TextColor3 = Color3.fromRGB(200, 200, 200)
        addOutput("Remote Spy durduruldu.", Color3.fromRGB(255, 0, 0))

        -- Bağlantıları temizle
        for remote, con in pairs(connections) do
            if remote ~= "_descendantAdded" then
                if type(con) == "table" and con.type == "function" then
                    -- OnServerInvoke veya Invoke'u eski haline döndür
                    if remote:IsA("RemoteFunction") then
                        remote.OnServerInvoke = con.old
                    elseif remote:IsA("BindableFunction") then
                        remote.Invoke = con.old
                    end
                else
                    con:Disconnect()
                end
        ...
        if connections._descendantAdded then
            connections._descendantAdded:Disconnect()
        end
        connections = {}
    end
    ...
    RemoteSpy.Init = function()
        createGui()
        -- Window açıldığında otomatik başlatma isteğe bağlı
    end
...
Main = (function()
	...
	Main.ModuleList = {"Explorer","Properties","ScriptViewer","Console","SaveInstance","ModelViewer","SettingsWindow","RemoteSpy"}
	Main.Elevated = false
	...
	Main.CreateMainGui = function()
        ...
        Main.CreateApp({Name = "Remote Spy", IconMap = Main.LargeIcons, Icon = "Executor", Window = RemoteSpy.Window})
